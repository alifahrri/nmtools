...
{ \
    ...
    auto result = nm::prelu(__VA_ARGS__); \
...
TEST_CASE("prelu(case1)" * doctest::test_suite("array::prelu"))
{
    ...
    #else
    PRELU_SUBCASE(case1, a_cs_fb);
    PRELU_SUBCASE(case1, a_cs_hb);
    PRELU_SUBCASE(case1, a_cs_db);

    PRELU_SUBCASE(case1, a_fs_fb);
    PRELU_SUBCASE(case1, a_fs_hb);
    PRELU_SUBCASE(case1, a_fs_db);

    PRELU_SUBCASE(case1, a_hs_fb);
    PRELU_SUBCASE(case1, a_hs_hb);
    PRELU_SUBCASE(case1, a_hs_db);

    PRELU_SUBCASE(case1, a_ds_fb);
    PRELU_SUBCASE(case1, a_ds_hb);
    PRELU_SUBCASE(case1, a_ds_db);

    PRELU_SUBCASE(case1, a_ls_fb);
    PRELU_SUBCASE(case1, a_ls_hb);
    PRELU_SUBCASE(case1, a_ls_db);
    #endif
}
...
TEST_CASE("conv3d(case1j)" * doctest::test_suite("array::conv3d"))
{
    [[maybe_unused]] auto dilation = nmtools::None;

    #if !defined(NMTOOLS_TESTING_GENERIC_NDARRAY)
    CONV3D_SUBCASE( case1j, input, weight, bias, stride_ct, padding_ct, dilation, groups_ct );
    CONV3D_SUBCASE( case1j, input_a, weight_a, bias_a, stride_ct, padding_ct, dilation, groups_ct );
    CONV3D_SUBCASE( case1j, input_f, weight_f, bias_f, stride_ct, padding_ct, dilation, groups_ct );
    CONV3D_SUBCASE( case1j, input_h, weight_h, bias_h, stride, padding, dilation, groups );
    CONV3D_SUBCASE( case1j, input_d, weight_d, bias_d, stride, padding, dilation, groups );
    #else
    #endif
}
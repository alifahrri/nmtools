...
{ \
    ...
    auto result = nmtools::conv3d(__VA_ARGS__); \
    NMTOOLS_ASSERT_EQUAL( nmtools::shape(result), nmtools::shape(expect::result) ); \
    NMTOOLS_ASSERT_CLOSE( result, expect::result ); \
}

TEST_CASE("conv3d(case1k)" * doctest::test_suite("array::conv3d"))
{
    #if !defined(NMTOOLS_TESTING_GENERIC_NDARRAY)
    CONV3D_SUBCASE( case1k, input, weight, bias, stride_ct, padding_ct, dilation_ct, groups_ct );
    CONV3D_SUBCASE( case1k, input_a, weight_a, bias_a, stride_ct, padding_ct, dilation_ct, groups_ct );
    CONV3D_SUBCASE( case1k, input_f, weight_f, bias_f, stride_ct, padding_ct, dilation_ct, groups_ct );
    CONV3D_SUBCASE( case1k, input_h, weight_h, bias_h, stride, padding, dilation, groups );
    CONV3D_SUBCASE( case1k, input_d, weight_d, bias_d, stride, padding, dilation, groups );
    #else
    #endif
}
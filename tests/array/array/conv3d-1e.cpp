...
TEST_CASE("conv3d(case1e)" * doctest::test_suite("array::conv3d"))
{
    [[maybe_unused]] auto padding = nmtools::None;
    [[maybe_unused]] auto dilation = nmtools::None;

    #if !defined(NMTOOLS_TESTING_GENERIC_NDARRAY)
    CONV3D_SUBCASE( case1e, input, weight, bias, stride_ct, padding, dilation, groups_ct );
    CONV3D_SUBCASE( case1e, input_a, weight_a, bias_a, stride_ct, padding, dilation, groups_ct );
    CONV3D_SUBCASE( case1e, input_f, weight_f, bias_f, stride_ct, padding, dilation, groups_ct );
    CONV3D_SUBCASE( case1e, input_h, weight_h, bias_h, stride, padding, dilation, groups );
    CONV3D_SUBCASE( case1e, input_d, weight_d, bias_d, stride, padding, dilation, groups );
    #else
    #endif
}
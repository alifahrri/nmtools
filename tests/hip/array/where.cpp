...
{ \
    ...
    auto result = na::where(__VA_ARGS__, hip::default_context()); \
    auto expect = na::where(__VA_ARGS__); \
    NMTOOLS_ASSERT_EQUAL( nm::shape(result), nm::shape(expect) ); \
    NMTOOLS_ASSERT_CLOSE( result, expect ); \
}

// TODO: fix runtime crash: (716): misaligned address
TEST_CASE("where(case1)" * doctest::test_suite("array::where") * doctest::skip())
{
...
// TODO: fix runtime crash: (716): misaligned address
TEST_CASE("where(case2)" * doctest::test_suite("array::where") * doctest::skip())
{

...
{ \
    ...
    auto result = na::where(__VA_ARGS__, na::sycl::default_context()); \
    auto expect = na::where(__VA_ARGS__); \
    NMTOOLS_ASSERT_EQUAL( nm::shape(result), nm::shape(expect) ); \
    NMTOOLS_ASSERT_CLOSE( result, expect ); \
}

// TODO: fix taking ptr of boolean array
// /workspaces/numeric_tools/include/nmtools/array/ndarray/ndarray.hpp:297:34: error: taking address of rvalue [-fpermissive]
//   297 |             return &self()->data_[0];
#if 0
...
TEST_CASE("where(case3)" * doctest::test_suite("array::where"))
{
    ...
    // WHERE_SUBCASE(case3, condition_ls_fb, x_ls_fb, y_ls_fb);
    // WHERE_SUBCASE(case3, condition_ls_hb, x_ls_hb, y_ls_hb);
    // WHERE_SUBCASE(case3, condition_ls_db, x_ls_db, y_ls_db);
}
#endif

TEST_CASE("where(case1)" * doctest::test_suite("array::where"))
{
    // WHERE_SUBCASE(case1b, condition, x, y);
    // WHERE_SUBCASE(case1b, condition_a, x_a, y_a);
    // WHERE_SUBCASE(case1b, condition_f, x_f, y_f);
    // WHERE_SUBCASE(case1b, condition_h, x_h, y_h);
    // WHERE_SUBCASE(case1b, condition_d, x_d, y_d);

    WHERE_SUBCASE(case1b, condition_cs_fb, x_cs_fb, y_cs_fb);
    WHERE_SUBCASE(case1b, condition_cs_hb, x_cs_hb, y_cs_hb);
    WHERE_SUBCASE(case1b, condition_cs_db, x_cs_db, y_cs_db);

    WHERE_SUBCASE(case1b, condition_fs_fb, x_fs_fb, y_fs_fb);
    WHERE_SUBCASE(case1b, condition_fs_hb, x_fs_hb, y_fs_hb);
    WHERE_SUBCASE(case1b, condition_fs_db, x_fs_db, y_fs_db);

    WHERE_SUBCASE(case1b, condition_hs_fb, x_hs_fb, y_hs_fb);
...
TEST_CASE("where(case2)" * doctest::test_suite("array::where"))
{
    ...
    // WHERE_SUBCASE(case2b, condition_a, x_a, y_a);
    // WHERE_SUBCASE(case2b, condition_f, x_f, y_f);
    // WHERE_SUBCASE(case2b, condition_h, x_h, y_h);
    // WHERE_SUBCASE(case2b, condition_d, x_d, y_d);

    WHERE_SUBCASE(case2b, condition_cs_fb, x_cs_fb, y_cs_fb);
    WHERE_SUBCASE(case2b, condition_cs_hb, x_cs_hb, y_cs_hb);
    WHERE_SUBCASE(case2b, condition_cs_db, x_cs_db, y_cs_db);

    WHERE_SUBCASE(case2b, condition_fs_fb, x_fs_fb, y_fs_fb);
    WHERE_SUBCASE(case2b, condition_fs_hb, x_fs_hb, y_fs_hb);
    WHERE_SUBCASE(case2b, condition_fs_db, x_fs_db, y_fs_db);

    WHERE_SUBCASE(case2b, condition_hs_fb, x_hs_fb, y_hs_fb);
    WHERE_SUBCASE(case2b, condition_hs_hb, x_hs_hb, y_hs_hb);
    WHERE_SUBCASE(case2b, condition_hs_db, x_hs_db, y_hs_db);

    WHERE_SUBCASE(case2b, condition_ds_fb, x_ds_fb, y_ds_fb);
    WHERE_SUBCASE(case2b, condition_ds_hb, x_ds_hb, y_ds_hb);
    WHERE_SUBCASE(case2b, condition_ds_db, x_ds_db, y_ds_db);

    // WHERE_SUBCASE(case2b, condition_ls_fb, x_ls_fb, y_ls_fb);
...
{
    // WHERE_SUBCASE(case3b, condition, x, y);
    // WHERE_SUBCASE(case3b, condition_a, x_a, y_a);
    // WHERE_SUBCASE(case3b, condition_f, x_f, y_f);
    // WHERE_SUBCASE(case3b, condition_h, x_h, y_h);
    // WHERE_SUBCASE(case3b, condition_d, x_d, y_d);

    WHERE_SUBCASE(case3b, condition_cs_fb, x_cs_fb, y_cs_fb);
    WHERE_SUBCASE(case3b, condition_cs_hb, x_cs_hb, y_cs_hb);
    WHERE_SUBCASE(case3b, condition_cs_db, x_cs_db, y_cs_db);

    WHERE_SUBCASE(case3b, condition_fs_fb, x_fs_fb, y_fs_fb);
    WHERE_SUBCASE(case3b, condition_fs_hb, x_fs_hb, y_fs_hb);
    WHERE_SUBCASE(case3b, condition_fs_db, x_fs_db, y_fs_db);

    WHERE_SUBCASE(case3b, condition_hs_fb, x_hs_fb, y_hs_fb);
    WHERE_SUBCASE(case3b, condition_hs_hb, x_hs_hb, y_hs_hb);
    WHERE_SUBCASE(case3b, condition_hs_db, x_hs_db, y_hs_db);

    WHERE_SUBCASE(case3b, condition_ds_fb, x_ds_fb, y_ds_fb);
    WHERE_SUBCASE(case3b, condition_ds_hb, x_ds_hb, y_ds_hb);
    WHERE_SUBCASE(case3b, condition_ds_db, x_ds_db, y_ds_db);

    // WHERE_SUBCASE(case3b, condition_ls_fb, x_ls_fb, y_ls_fb);

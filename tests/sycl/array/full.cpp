...
#define SYCL_FULL_SUBCASE(...) \
{ \
    auto expect = na::full(__VA_ARGS__); \
    auto result = na::full(__VA_ARGS__, na::sycl::default_context()); \
...
{
    auto shape = nmtools_array{128};
    SYCL_FULL_SUBCASE(shape,(uint8_t)3);
}

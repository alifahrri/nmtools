...
#define SYCL_CUMSUM_SUBCASE(...) \
{ \
    auto expect = na::cumsum(__VA_ARGS__); \
    auto result = na::cumsum(__VA_ARGS__, na::sycl::default_context()); \
...
{ \
    ...
    SYCL_CUMSUM_SUBCASE(__VA_ARGS__); \
}
...
TEST_CASE("cumsum(case1)" * doctest::test_suite("array::cumsum"))
{
    ...
    {
        auto axis = 0;
        SYCL_CUMSUM_SUBCASE(input,axis,dtype);
    }
}

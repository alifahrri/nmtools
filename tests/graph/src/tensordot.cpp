...
TEST_CASE("get_computational_graph(tensordot)" * doctest::test_suite("transform"))
{
    auto gen   = nm::random_engine();
    auto dtype = nm::float32;

    auto lhs = nm::random(array{2,3,4,5},dtype,gen);
    auto rhs = nm::random(array{3,5,4,2},dtype,gen);
    auto axes = nmtools_tuple{
        nmtools_array{2,3},
        nmtools_array{-2,-3},
    };

    auto res = view::tensordot(lhs,rhs,axes);

...
#define UNROLL_DIVIDE_SUBCASE(case_name, ...) \
SUBCASE(#case_name) \
{ \
    NMTOOLS_TESTING_USE_CASE(view, divide, case_name); \
    using namespace args; \
    auto result = nmtools::divide(__VA_ARGS__, nmtools::Unroll); \
    NMTOOLS_STATIC_CHECK_TRAIT( nmtools::meta::is_ndarray, decltype(result) ); \
    NMTOOLS_ASSERT_EQUAL( ::nm::shape(result), expect::shape ); \
    NMTOOLS_ASSERT_CLOSE( result, expect::result ); \
}

TEST_CASE("divide(case1)" * doctest::test_suite("unroll_context::divide"))
{
    UNROLL_DIVIDE_SUBCASE( case1, a, b );
    UNROLL_DIVIDE_SUBCASE( case1, a_f, b_f );

    UNROLL_DIVIDE_SUBCASE( case1, a_cs_fb, b_cs_fb );
    UNROLL_DIVIDE_SUBCASE( case1, a_cs_hb, b_cs_hb );
    UNROLL_DIVIDE_SUBCASE( case1, a_cs_db, b_cs_db );
}

TEST_CASE("divide(case2)" * doctest::test_suite("unroll_context::divide"))
{
    UNROLL_DIVIDE_SUBCASE( case2, a, b );
    UNROLL_DIVIDE_SUBCASE( case2, a_f, b );

    UNROLL_DIVIDE_SUBCASE( case2, a_cs_fb, b );
    UNROLL_DIVIDE_SUBCASE( case2, a_cs_hb, b );
    UNROLL_DIVIDE_SUBCASE( case2, a_cs_db, b );
}

...
#define UNROLL_SIN_SUBCASE(case_name, ...) \
SUBCASE(#case_name) \
{ \
    NMTOOLS_TESTING_USE_CASE(view, sin, case_name); \
    using namespace args; \
    auto result = nmtools::sin(__VA_ARGS__, nmtools::Unroll); \
    NMTOOLS_STATIC_CHECK_TRAIT( nmtools::meta::is_ndarray, decltype(result) ); \
    NMTOOLS_ASSERT_EQUAL( ::nm::shape(result), expect::shape ); \
    NMTOOLS_ASSERT_CLOSE( result, expect::result ); \
}

TEST_CASE("sin(case1)" * doctest::test_suite("unroll_context::sin"))
{
    UNROLL_SIN_SUBCASE( case1, a );
    UNROLL_SIN_SUBCASE( case1, a_f );

    UNROLL_SIN_SUBCASE( case1, a_cs_fb );
    UNROLL_SIN_SUBCASE( case1, a_cs_hb );
    UNROLL_SIN_SUBCASE( case1, a_cs_db );
}

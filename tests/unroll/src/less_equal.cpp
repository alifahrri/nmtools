...
#define UNROLL_LESS_EQUAL_SUBCASE(case_name, ...) \
SUBCASE(#case_name) \
{ \
    NMTOOLS_TESTING_USE_CASE(view, less_equal, case_name); \
    using namespace args; \
    auto result = nmtools::less_equal(__VA_ARGS__, nmtools::Unroll); \
    NMTOOLS_STATIC_CHECK_TRAIT( nmtools::meta::is_ndarray, decltype(result) ); \
    NMTOOLS_ASSERT_EQUAL( ::nm::shape(result), expect::shape ); \
    NMTOOLS_ASSERT_CLOSE( result, expect::result ); \
}

TEST_CASE("less_equal(case1)" * doctest::test_suite("unroll_context::less_equal"))
{
    UNROLL_LESS_EQUAL_SUBCASE( case1, a, b );
    UNROLL_LESS_EQUAL_SUBCASE( case1, a_f, b_f );

    UNROLL_LESS_EQUAL_SUBCASE( case1, a_cs_fb, b_cs_fb );
    UNROLL_LESS_EQUAL_SUBCASE( case1, a_cs_hb, b_cs_hb );
    UNROLL_LESS_EQUAL_SUBCASE( case1, a_cs_db, b_cs_db );
}

TEST_CASE("less_equal(case2)" * doctest::test_suite("unroll_context::less_equal"))
{
    UNROLL_LESS_EQUAL_SUBCASE( case2, a, b );
    UNROLL_LESS_EQUAL_SUBCASE( case2, a_f, b );

    UNROLL_LESS_EQUAL_SUBCASE( case2, a_cs_fb, b );
    UNROLL_LESS_EQUAL_SUBCASE( case2, a_cs_hb, b );
    UNROLL_LESS_EQUAL_SUBCASE( case2, a_cs_db, b );
}

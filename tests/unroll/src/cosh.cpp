...
{ \
    ...
    auto result = nmtools::cosh(__VA_ARGS__, nmtools::Unroll); \
    NMTOOLS_STATIC_CHECK_TRAIT( nmtools::meta::is_ndarray, decltype(result) ); \
    NMTOOLS_ASSERT_EQUAL( ::nm::shape(result), expect::shape ); \
    NMTOOLS_ASSERT_CLOSE( result, expect::result ); \
}

TEST_CASE("cosh(case1)" * doctest::test_suite("unroll_context::cosh"))
{
    UNROLL_COSH_SUBCASE( case1, a );
    UNROLL_COSH_SUBCASE( case1, a_f );

    UNROLL_COSH_SUBCASE( case1, a_cs_fb );
    UNROLL_COSH_SUBCASE( case1, a_cs_hb );
    UNROLL_COSH_SUBCASE( case1, a_cs_db );
}

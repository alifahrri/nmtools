...
#define UNROLL_PROD_SUBCASE(case_name, ...) \
SUBCASE(#case_name) \
{ \
    NMTOOLS_TESTING_USE_CASE(view, reduce_multiply, case_name); \
    using namespace args; \
    auto result = nmtools::prod(__VA_ARGS__, nmtools::Unroll); \
    NMTOOLS_STATIC_CHECK_TRAIT( nmtools::meta::is_ndarray, decltype(result) ); \
    NMTOOLS_ASSERT_EQUAL( nm::shape(result), expect::shape ); \
    NMTOOLS_ASSERT_CLOSE( result, expect::result ); \
}

TEST_CASE("prod(case1)" * doctest::test_suite("unroll_context::prod"))
{
    UNROLL_PROD_SUBCASE( case1, a, axis_ct );

    UNROLL_PROD_SUBCASE( case1, a_cs_fb, axis_ct );
    UNROLL_PROD_SUBCASE( case1, a_cs_hb, axis_ct );
    UNROLL_PROD_SUBCASE( case1, a_cs_db, axis_ct );
}

TEST_CASE("prod(case2)" * doctest::test_suite("unroll_context::prod"))
{
    UNROLL_PROD_SUBCASE( case2, a, axis_ct );

    UNROLL_PROD_SUBCASE( case2, a_cs_fb, axis_ct );
    UNROLL_PROD_SUBCASE( case2, a_cs_hb, axis_ct );
    UNROLL_PROD_SUBCASE( case2, a_cs_db, axis_ct );
}

TEST_CASE("prod(case3)" * doctest::test_suite("unroll_context::prod"))
{
    UNROLL_PROD_SUBCASE( case3, a, axis_ct );

    UNROLL_PROD_SUBCASE( case3, a_cs_fb, axis_ct );
    UNROLL_PROD_SUBCASE( case3, a_cs_hb, axis_ct );
    UNROLL_PROD_SUBCASE( case3, a_cs_db, axis_ct );
}

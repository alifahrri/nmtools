...
#define UNROLL_LOG10_SUBCASE(case_name, ...) \
SUBCASE(#case_name) \
{ \
    NMTOOLS_TESTING_USE_CASE(view, log10, case_name); \
    using namespace args; \
    auto result = nmtools::log10(__VA_ARGS__, nmtools::Unroll); \
    NMTOOLS_STATIC_CHECK_TRAIT( nmtools::meta::is_ndarray, decltype(result) ); \
    NMTOOLS_ASSERT_EQUAL( ::nm::shape(result), expect::shape ); \
    NMTOOLS_ASSERT_CLOSE( result, expect::result ); \
}

TEST_CASE("log10(case1)" * doctest::test_suite("unroll_context::log10"))
{
    UNROLL_LOG10_SUBCASE( case1, a );
    UNROLL_LOG10_SUBCASE( case1, a_f );

    UNROLL_LOG10_SUBCASE( case1, a_cs_fb );
    UNROLL_LOG10_SUBCASE( case1, a_cs_hb );
    UNROLL_LOG10_SUBCASE( case1, a_cs_db );
}

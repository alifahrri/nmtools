...
NMTOOLS_TESTING_DECLARE_CASE(logical_not)
{
    NMTOOLS_TESTING_DECLARE_ARGS(case1)
    {
        ...
        auto x_fixed_vector = cast<na::fixed_vector<bool,5>>(x);
        auto x_dynamic_vector = cast<na::dynamic_vector<bool>>(x);
    }
...
TEST_CASE("logical_not(dynamic_vector)" * doctest::test_suite("index::logical_not"))
{
    NMTOOLS_TESTING_DECLARE_NS(logical_not, case1);
    auto result = nm::index::logical_not(args::x_dynamic_vector);
    auto expected = expect::x_dynamic_vector;
    NMTOOLS_ASSERT_EQUAL( result, expected );
}
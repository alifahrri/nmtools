...
TEST_CASE("eval(ufunc)" * doctest::test_suite("eval"))
{
    SUBCASE("sin")
    {
        ...
        {
            using view_t = view::decorator_t< view::ufunc_t, view::sin_t, std::vector<float> >;
            using eval_t = meta::resolve_optype_t<na::eval_t, view_t, none_t>;
            using expected_t = std::vector<float>;
            NMTOOLS_STATIC_CHECK_IS_SAME( eval_t, expected_t );
        }
        {
            using view_t = view::decorator_t< view::ufunc_t, view::sin_t, std::array<int,4> >;
            using eval_t = meta::resolve_optype_t< na::eval_t, view_t, none_t >;
            using expected_t = std::array<double,4>;
            NMTOOLS_STATIC_CHECK_IS_SAME( eval_t, expected_t );
        }
        {
    ...
    {
        {
            ...
            using view_t = view::decorator_t< view::ufunc_t, op_t, std::vector<int>, std::vector<int> >;
            using eval_t = meta::resolve_optype_t<na::eval_t, view_t, none_t>;
        ...
        {
            using op_t   = view::add_t<>;
            using view_t = view::decorator_t< view::ufunc_t, op_t, std::array<int,4>, std::array<int,4> >;
            using eval_t = meta::resolve_optype_t<na::eval_t, view_t, none_t>;
            using expected_t = std::array<int,4>;
            NMTOOLS_STATIC_CHECK_IS_SAME( eval_t, expected_t );
        }
        {
            using op_t   = view::add_t<>;
    ...
    SUBCASE("clip")
    {
        ...
        }
        {
            ...
            >;
            using eval_t = meta::resolve_optype_t< na::eval_t, view_t, none_t >;
            using expected_t = std::array<std::array<float,2>,3>;
            NMTOOLS_STATIC_CHECK_IS_SAME( eval_t, expected_t );
        }
    }
}
...
{
    {
        ...
        using eval_t = meta::resolve_optype_t< na::eval_t, view_t, none_t >;
        using expected_t = std::array<std::array<std::array<int, 3ul>, 3ul>, 2ul>;
        NMTOOLS_STATIC_CHECK_IS_SAME( eval_t, expected_t );
        // static_assert( meta::is_fixed_size_ndarray_v<view_t> );
    }
    {

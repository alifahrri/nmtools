#include "nmtools/array/ufuncs/log10.hpp"
#include "nmtools/context/default.hpp"
#include "nmtools/testing/data/array/log10.hpp"
#include "nmtools/testing/doctest.hpp"

namespace nm = nmtools;

#define OBJECT_LOG10_SUBCASE(case_name, a, ...) \
SUBCASE(#case_name) \
{ \
    NMTOOLS_TESTING_USE_CASE(view, log10, case_name); \
    using namespace args; \
    auto object = nm::array(a,nm::Object); \
    auto result = object.log10(__VA_ARGS__); \
    NMTOOLS_ASSERT_EQUAL( ::nm::shape(result), expect::shape ); \
    NMTOOLS_ASSERT_CLOSE( result, expect::result ); \
}

#define LOG10_SUBCASE(case_name, ...) \
SUBCASE(#case_name) \
{ \
    NMTOOLS_TESTING_USE_CASE(view, log10, case_name); \
    using namespace args; \
    auto result = nm::log10(__VA_ARGS__); \
    NMTOOLS_ASSERT_EQUAL( ::nm::shape(result), expect::shape ); \
    NMTOOLS_ASSERT_CLOSE( result, expect::result ); \
}

TEST_CASE("log10(case1)" * doctest::test_suite("array::log10"))
{
    LOG10_SUBCASE( case1,   a);
    LOG10_SUBCASE( case1, a_a);
    LOG10_SUBCASE( case1, a_f);
    LOG10_SUBCASE( case1, a_d);
    LOG10_SUBCASE( case1, a_h);
    
    OBJECT_LOG10_SUBCASE( case1,   a);
    OBJECT_LOG10_SUBCASE( case1, a_a);
    OBJECT_LOG10_SUBCASE( case1, a_f);
    OBJECT_LOG10_SUBCASE( case1, a_d);
    OBJECT_LOG10_SUBCASE( case1, a_h);
}
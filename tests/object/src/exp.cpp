...
#define OBJECT_EXP_SUBCASE(case_name, a, ...) \
SUBCASE(#case_name) \
{ \
    NMTOOLS_TESTING_USE_CASE(view, exp, case_name); \
    using namespace args; \
    auto object = nmtools::array(a,nm::Object); \
    auto result = object.exp(__VA_ARGS__); \
...
{ \
    NMTOOLS_TESTING_USE_CASE(view, exp, case_name); \
    using namespace args; \
    auto result = nmtools::exp(__VA_ARGS__); \
    NMTOOLS_ASSERT_EQUAL( ::nm::shape(result), expect::shape ); \
    NMTOOLS_ASSERT_CLOSE( result, expect::result ); \
}
...
TEST_CASE("exp(case1)" * doctest::test_suite("array::exp"))
{
    ...
    OBJECT_EXP_SUBCASE( case1,   a);
    OBJECT_EXP_SUBCASE( case1, a_a);
    OBJECT_EXP_SUBCASE( case1, a_f);
    OBJECT_EXP_SUBCASE( case1, a_d);
    OBJECT_EXP_SUBCASE( case1, a_h);
}
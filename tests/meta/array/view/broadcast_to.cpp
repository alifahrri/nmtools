...
TEST_CASE("is_fixed_shape" * doctest::test_suite("view"))
{
    SUBCASE("broadcast_to")
    {
        // reduce add, as needed by mean view
        {
            ...
            using view_t  = view::decorator_t<view::reduce_t, op_t, array_t, decltype(0_ct), none_t, false_type, none_t >;
            using shape_t = nmtools_tuple<integral_constant<long unsigned int, 2> >;
            using broadcasted_t = decltype(nm::unwrap(view::broadcast_to(declval(view_t),declval(shape_t))));

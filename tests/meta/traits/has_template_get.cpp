...
{
    {
        using arg_t = const nmtools_tuple<int,int>&;
        STATIC_CHECK(( meta::has_template_get_v<arg_t,1> ));
    }
    {
        using arg_t = const nmtools_array<int,4>&;
        STATIC_CHECK(( meta::has_template_get_v<arg_t,1> ));
    }
    ...
    // error: static_assert failed due to requirement '0UL < tuple_size<nmtools_tuple<> >::value' "tuple index is in range"
    //   static_assert(__i < tuple_size<tuple<>>::value
    // {
    //     using arg_t = nmtools_tuple<int,int,int,int>;
    //     STATIC_CHECK(( !meta::has_template_get_v<arg_t,4> ));
    // }
}
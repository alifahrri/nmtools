...
using Attribute  = nrt::Attribute<>;
using Attributes = nrt::Attributes<>;
using Node = nrt::Node<>;
using nrt::Type, nrt::Layout;
...
TEST_CASE("attribute" * doctest::test_suite("runtime"))
{
    {
...
TEST_CASE("attributes" * doctest::test_suite("runtime"))
{
    {
...
TEST_CASE("node" * doctest::test_suite("runtime"))
{
    // buffer
    {
    ...
    // indexing
    {
        ...
        {
            auto key = nmtools_string("indexer.args.") + utl::to_string(0).c_str();
...
TEST_CASE("node(composition)" * doctest::test_suite("runtime"))
{
    {
...
TEST_CASE("to_value(buffer)" * doctest::test_suite("runtime"))
{
    ...
    auto NODE = nrt::to_value(node);
    NMTOOLS_ASSERT_EQUAL( (nm::is_same_v<nm::remove_cvref_t<decltype(NODE)>,Node>), true );
...
TEST_CASE("to_value(add)" * doctest::test_suite("runtime"))
{
    auto gen = nm::random_engine();
    auto dtype = nm::float32;
...
#if 0
TEST_CASE("to_value(reshape)" * doctest::test_suite("runtime"))
{
    auto gen = nm::random_engine();
    auto dtype = nm::float32;
...
TEST_CASE("to_value(sum)" * doctest::test_suite("runtime"))
{
    auto gen = nm::random_engine();
    auto dtype = nm::float32;
...
TEST_CASE("to_value(sum)" * doctest::test_suite("runtime"))
{
    ...
    NMTOOLS_ASSERT_EQUAL( (nm::is_same_v<nm::remove_cvref_t<decltype(NODE)>,Node>), true );
    NMTOOLS_ASSERT_EQUAL( NODE.is_compute(), true );
    NMTOOLS_ASSERT_EQUAL( NODE.dim(), 0 );
    NMTOOLS_ASSERT_EQUAL( NODE.max_dim(), -1 );
...
TEST_CASE("to_value(cos)" * doctest::test_suite("runtime"))
{
    auto gen = nm::random_engine();
    auto dtype = nm::float32;
...
TEST_CASE("to_value(cos)" * doctest::test_suite("runtime"))
{
    ...
    NMTOOLS_ASSERT_EQUAL( (nm::is_same_v<nm::remove_cvref_t<decltype(NODE)>,Node>), true );
    NMTOOLS_ASSERT_EQUAL( NODE.is_compute(), true );
    NMTOOLS_ASSERT_EQUAL( NODE.dim(), 3 );
...
TEST_CASE("to_value(input)" * doctest::test_suite("runtime"))
{
    auto input = 3;
...
TEST_CASE("to_value(functors)" * doctest::test_suite("runtime"))
{
    auto functors = fn::reshape[array{3,4}] * fn::flatten;

    auto node = fn::compute_node_t{functors,nmtools_tuple{},array{3,4},nm::float32};
    // static_assert( nm::is_functor_composition_v<decltype(node.functor)> );
    // static_assert( nm::len_v<nm::remove_cvref_t<decltype(node.functor.functors)>> == 2 );
    auto NODE = nrt::to_value(node);
    NMTOOLS_ASSERT_EQUAL( (nm::is_same_v<nm::remove_cvref_t<decltype(NODE)>,Node>), true );
...
TEST_CASE("to_value(combinator)" * doctest::test_suite("runtime"))
{
...
TEST_CASE("to_value(functor)" * doctest::test_suite("runtime"))
{
...
TEST_CASE("node(buffer)" * doctest::test_suite("runtime"))
{
    auto gen   = nm::random_engine();
    auto dtype = nm::float32;
    ...
    auto cnode = nm::to_value_v<decltype(node)>;
    nrt::Node<> NODE = cnode;
    NMTOOLS_ASSERT_EQUAL( (nm::is_same_v<nm::remove_cvref_t<decltype(NODE)>,Node>), true );
    NMTOOLS_ASSERT_EQUAL( NODE.is_buffer(), true );
    NMTOOLS_ASSERT_EQUAL( NODE.dim(), nm::len(shape) );
...
TEST_CASE("node(buffer)" * doctest::test_suite("runtime"))
{
    ...
    NMTOOLS_ASSERT_EQUAL( NODE.is_buffer(), true );
    // NMTOOLS_ASSERT_EQUAL( NODE.dim(), 2 );
    NMTOOLS_ASSERT_EQUAL( NODE.dim(), -1 ); // because dim is known at runtime
    NMTOOLS_ASSERT_EQUAL( NODE.max_dim(), -1 );
    NMTOOLS_ASSERT_EQUAL( NODE.is_num(), 0 );
    CHECK( NODE.kind() == fn::Kind::BUFFERED );
    CHECK( NODE.dtype() == fn::Type::Int8 );
...
TEST_CASE("node(input)" * doctest::test_suite("runtime"))
{
    auto input = 3;
...
TEST_CASE("node(add)" * doctest::test_suite("runtime"))
{
    auto gen = nm::random_engine();
    auto dtype = nm::float32;
...
TEST_CASE("node(sum)" * doctest::test_suite("runtime"))
{
    auto gen = nm::random_engine();
    auto dtype = nm::float32;
...
TEST_CASE("node(sum)" * doctest::test_suite("runtime"))
{
    ...
    NMTOOLS_ASSERT_EQUAL( NODE.is_compute(), true );
    // NMTOOLS_ASSERT_EQUAL( NODE.dim(), 0 );
    NMTOOLS_ASSERT_EQUAL( NODE.dim(), -1 ); // why -1? because is num?
    NMTOOLS_ASSERT_EQUAL( NODE.max_dim(), -1 );
    NMTOOLS_ASSERT_EQUAL( NODE.is_num(), 1 );
    CHECK( NODE.kind() == fn::Kind::REDUCE );
    CHECK( NODE.dtype() == fn::Type::Float32 );
...
TEST_CASE("node(cos)" * doctest::test_suite("runtime"))
{
    auto gen = nm::random_engine();
    auto dtype = nm::float32;
...
TEST_CASE("node(cos)" * doctest::test_suite("runtime"))
{
    ...
    NMTOOLS_ASSERT_EQUAL( NODE.is_compute(), true );
    // NMTOOLS_ASSERT_EQUAL( NODE.dim(), 3 );
    NMTOOLS_ASSERT_EQUAL( NODE.dim(), -1 ); // only known at runtime
    NMTOOLS_ASSERT_EQUAL( NODE.max_dim(), 4 );
    NMTOOLS_ASSERT_EQUAL( NODE.is_num(), 0 );
    // we don't know the shape at compile time, only runtime
    // NMTOOLS_ASSERT_EQUAL( NODE.shape(), src_shape );
    CHECK( NODE.kind() == fn::Kind::UNARY_UFUNC );
    CHECK( NODE.dtype() == fn::Type::Float32 );
    CHECK_MESSAGE( true, node.to_string() );

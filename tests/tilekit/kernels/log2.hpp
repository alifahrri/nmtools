#ifndef NMTOOLS_TESTS_KERNELS_LOG2_HPP
#define NMTOOLS_TESTS_KERNELS_LOG2_HPP
        ...
        for (nm_size_t i=0; i<n_iter; i++) {
            ...
            auto result = [&](){
                if constexpr (use_utl) {
                    return block.ufunc(ops::utl::log2);
                } else {
                    return block.log2();
                }
            }();
...
constexpr inline auto log2_kernel = log2_kernel_t{};
constexpr inline auto log2_utl_kernel = log2_kernel_t<true>{};

#endif // NMTOOLS_TESTS_KERNELS_LOG2_HPP
#ifndef NMTOOLS_TESTS_KERNELS_LOG10_HPP
#define NMTOOLS_TESTS_KERNELS_LOG10_HPP
        ...
        for (nm_size_t i=0; i<n_iter; i++) {
            ...
            auto result = [&](){
                if constexpr (use_utl) {
                    return block.ufunc(ops::utl::log10);
                } else {
                    return block.log10();
                }
            }();
...
constexpr inline auto log10_kernel = log10_kernel_t{};
constexpr inline auto log10_utl_kernel = log10_kernel_t<true>{};

#endif // NMTOOLS_TESTS_KERNELS_LOG10_HPP
#ifndef NMTOOLS_TESTS_KERNELS_SIN_HPP
#define NMTOOLS_TESTS_KERNELS_SIN_HPP
        ...
        for (nm_size_t i=0; i<n_iter; i++) {
            ...
            auto result = [&](){
                if constexpr (use_utl) {
                    return block.ufunc(ops::utl::sin);
                } else {
                    return block.sin();
                }
            }();
...
constexpr inline auto sin_kernel = sin_kernel_t{};
constexpr inline auto sin_utl_kernel = sin_kernel_t<true>{};

#endif // NMTOOLS_TESTS_KERNELS_SIN_HPP
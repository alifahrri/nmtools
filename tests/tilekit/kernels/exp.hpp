#ifndef NMTOOLS_TESTS_KERNELS_EXP_HPP
#define NMTOOLS_TESTS_KERNELS_EXP_HPP
        ...
        for (nm_size_t i=0; i<n_iter; i++) {
            ...
            auto result = [&](){
                if constexpr (use_utl) {
                    return block.ufunc(ops::utl::exp);
                } else {
                    return block.exp();
                }
            }();
...
constexpr inline auto exp_kernel = exp_kernel_t{};
constexpr inline auto exp_utl_kernel = exp_kernel_t<true>{};

#endif // NMTOOLS_TESTS_KERNELS_EXP_HPP
...
{ \
    ...
    auto result = nmtools::array::vstack(__VA_ARGS__,na::cuda::default_context()); \
    NMTOOLS_ASSERT_EQUAL( nm::shape(result), nm::shape(expect::result) ); \
    NMTOOLS_ASSERT_CLOSE( result, expect::result ); \
}

TEST_CASE("vstack(case1)" * doctest::test_suite("array::vstack"))
{
    // TODO: support the following types for cuda

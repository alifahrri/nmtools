#ifndef NMTOOLS_ARRAY_EVAL_OPENCL_KERNELS_EXPAND_DIMS_HPP
...
#define nmtools_cl_kernel_name(out_type,inp_type) expand_dims_##out_type##_##inp_type
#define nmtools_cl_kernel_name_str(out_type,inp_type) nm_stringify(expand_dims_##out_type##_##inp_type)
...
{ \
    ...
    auto result = view::expand_dims(input,axis); \
...
extern unsigned char nm_cl_expand_dims_spv[];
extern unsigned int nm_cl_expand_dims_spv_len;

namespace nmtools::array::opencl
{
    template <typename...args_t>
    struct kernel_t<
        view::decorator_t<view::expand_dims_t,args_t...>
    > {
        using view_t = view::decorator_t<view::expand_dims_t,args_t...>;
        ...
        static auto get_spirv()
        {
            ...
            auto spirv = vector();
            spirv.resize(nm_cl_expand_dims_spv_len);
            memcpy(spirv.data(),nm_cl_expand_dims_spv,sizeof(unsigned char)*nm_cl_expand_dims_spv_len);
            return spirv;
        }
...
#endif // NMTOOLS_ARRAY_EVAL_OPENCL_KERNELS_EXPAND_DIMS_HPP
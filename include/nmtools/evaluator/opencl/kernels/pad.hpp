#ifndef NMTOOLS_ARRAY_EVAL_OPENCL_KERNELS_PAD_HPP
...
#define nmtools_cl_kernel_name_str(out_type,inp_type) nm_stringify(pad##_##out_type##_##inp_type)
...
namespace nmtools::array::opencl
{
    ...
    struct kernel_t<
        view::decorator_t<view::pad_t,args_t...>
    > {
        using view_t = view::decorator_t<view::pad_t,args_t...>;
        ...
        static auto get_spirv()
        {
            ...
            auto spirv = vector();
            spirv.resize(nm_cl_pad_spv_len);
            memcpy(spirv.data(),nm_cl_pad_spv,sizeof(unsigned char) * nm_cl_pad_spv_len);
            return spirv;
        }
        ...
        auto eval(output_t& output)
        {
            ...
            auto pad_width = view.pad_width;
            auto pad_size  = nmtools::len(pad_width);
            auto pad_value = view.pad_value;
            ...
            auto pad_width_buffer = context->create_buffer(index::cast<nm_cl_index_t>(pad_width));
...
#endif // NMTOOLS_ARRAY_EVAL_OPENCL_KERNELS_PAD_HPP
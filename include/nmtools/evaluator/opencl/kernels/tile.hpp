#ifndef NMTOOLS_ARRAY_EVAL_OPENCL_KERNELS_TILE_HPP
...
kernel void nmtools_cl_kernel_name(out_type,inp_type) \
    ...
    , global const nm_cl_index_t* reps_ptr \
    , const nm_cl_size_t out_dim \
    , const nm_cl_size_t inp_dim \
    , const nm_cl_size_t reps_len \
    ) \
{ \
    auto reps  = na::create_vector(reps_ptr,reps_len); \
    ...
    auto result = view::tile(input,reps); \
...
extern unsigned char nm_cl_tile_spv[];
extern unsigned int nm_cl_tile_spv_len;

namespace nmtools::array::opencl
{
    template <typename...args_t>
    struct kernel_t<
        view::decorator_t<view::tile_t,args_t...>
    > {
        using view_t = view::decorator_t<view::tile_t,args_t...>;
        ...
        static auto get_spirv()
        {
            ...
            auto spirv = vector();
            spirv.resize(nm_cl_tile_spv_len);
            memcpy(spirv.data(),nm_cl_tile_spv,sizeof(unsigned char)*nm_cl_tile_spv_len);
            return spirv;
        }
        ...
        auto eval(output_t& output)
        {
            ...
            // assume index array
            auto reps = index::cast<nm_cl_index_t>(view.reps);
            auto reps_len = nmtools::len(reps);

            auto reps_buffer = context->create_buffer(reps);
...
#endif // NMTOOLS_ARRAY_EVAL_OPENCL_KERNELS_TILE_HPP
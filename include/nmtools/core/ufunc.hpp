...
namespace nmtools::view
{
    ...
    constexpr auto _ufunc(op_t op, const tuple<arrays_t...>& func, index_sequence<Is...>)
    {
        // help deduction
    ...
    {
        return _ufunc(op,func,make_index_sequence_v<sizeof...(arrays_t)>);
    }; // _ufunc
    ...
    constexpr auto ufunc(op_t op, const array_t& array, const arrays_t&...arrays)
    {
        ...
        if constexpr (is_maybe_v<array_t>) {
        ...
        } else if constexpr (is_either_v<array_t>) {
            ...
            using lhs_t = get_either_left_t<array_t>;
            using rhs_t = get_either_right_t<array_t>;
            using lhs_ufunc_t = decltype(ufunc(op,declval<lhs_t>(),arrays...));
            using rhs_ufunc_t = decltype(ufunc(op,declval<rhs_t>(),arrays...));
            using result_t = replace_either_t<array_t,lhs_ufunc_t,rhs_ufunc_t>;
            if (auto lptr = nmtools::get_if<lhs_t>(&array)) {
                static_assert( is_pointer_v<decltype(lptr)>
                    , "nmtools internal error" );
                static_assert( is_same_v<remove_cvref_t<decltype(*lptr)>,lhs_t>
                    , "nmtools internal error" );
                ...
                static_assert( is_pointer_v<decltype(rptr)>
                    , "nmtools internal error" );
                static_assert( is_same_v<remove_cvref_t<decltype(*rptr)>,rhs_t>
                    , "nmtools internal error" );
                return result_t{ufunc(op,*rptr,arrays...)};
            }
        } else if constexpr (is_num_v<array_t> && (is_num_v<arrays_t> && ...)) {
            ...
            constexpr auto n_args = len_v<b_arrays_t>;
            ...
            using view_t = ufunc_t<op_t,array_t>;
    ...
    constexpr auto unary_ufunc(op_t op, const array_t& array, op_args_t...op_args)
    {
        if constexpr (is_maybe_v<array_t>) {
            ...
            using lhs_t = get_either_left_t<array_t>;
            using rhs_t = get_either_right_t<array_t>;
            using lhs_ufunc_t = decltype(unary_ufunc(op,declval<lhs_t>(),op_args...));
            using rhs_ufunc_t = decltype(unary_ufunc(op,declval<rhs_t>(),op_args...));
            using return_t = replace_either_t<array_t,lhs_ufunc_t,rhs_ufunc_t>;
            if (auto lptr = nmtools::get_if<lhs_t>(&array)) {
    ...
    {
        if constexpr (is_maybe_v<lhs_t>) {
            using result_type = decltype(binary_ufunc<broadcast_enable>(op,*lhs,rhs));
            using return_type = conditional_t<is_maybe_v<result_type>
                , result_type, nmtools_maybe<result_type>
            >;
            return (static_cast<bool>(lhs)
                ? return_type{binary_ufunc<broadcast_enable>(op,*lhs,rhs)}
                : return_type{Nothing}
            );
        } else if constexpr (is_maybe_v<rhs_t>) {
            using result_type = decltype(binary_ufunc<broadcast_enable>(op,lhs,*rhs));
            using return_type = conditional_t<is_maybe_v<result_type>
                , result_type, nmtools_maybe<result_type>
            >;
        ...
        } else if constexpr (is_either_v<lhs_t>) {
            using l_lhs_t = get_either_left_t<lhs_t>;
            using l_rhs_t = get_either_right_t<lhs_t>;
            using lhs_ufunc_t = decltype(binary_ufunc<broadcast_enable>(op,declval<lhs_t>(),rhs));
            using rhs_ufunc_t = decltype(binary_ufunc<broadcast_enable>(op,declval<rhs_t>(),rhs));
            using result_t = replace_either_t<lhs_t,lhs_ufunc_t,rhs_ufunc_t>;
            if (auto lptr = nmtools::get_if<l_lhs_t>(&lhs)) {
                return result_t{binary_ufunc<broadcast_enable>(op,*lptr,rhs)};
            } else {
                auto rptr = nmtools::get_if<l_rhs_t>(&lhs);
                return result_t{binary_ufunc<broadcast_enable>(op,*rptr,rhs)};
            }
        } else if constexpr (is_either_v<rhs_t>) {
            using r_lhs_t = get_either_left_t<rhs_t>;
            using r_rhs_t = get_either_right_t<rhs_t>;
            using lhs_ufunc_t = decltype(binary_ufunc<broadcast_enable>(op,lhs,declval<r_lhs_t>()));
            using rhs_ufunc_t = decltype(binary_ufunc<broadcast_enable>(op,lhs,declval<r_rhs_t>()));
            using result_t = replace_either_t<rhs_t,lhs_ufunc_t,rhs_ufunc_t>;
            if (auto lptr = nmtools::get_if<r_lhs_t>(&rhs)) {
                return result_t{binary_ufunc<broadcast_enable>(op,lhs,*lptr)};
            } else {
                auto rptr = nmtools::get_if<r_rhs_t>(&rhs);
                return result_t{binary_ufunc<broadcast_enable>(op,lhs,*rptr)};
            }
        } else if constexpr (is_num_v<lhs_t> && is_num_v<rhs_t>) {
    ...
    constexpr auto binary_ufunc(op_t op, const arrays_t& arrays)
    {
        if constexpr (is_maybe_v<arrays_t>) {
            using arrays_type = get_maybe_type_t<arrays_t>;
            using result_type = decltype(binary_ufunc(op,declval<arrays_type>()));
            using return_type = conditional_t<is_maybe_v<result_type>,result_type,nmtools_maybe<result_type>>;
            return (arrays?
            ...
            static_assert( len_v<arrays_t> == 2
    ...
    constexpr auto broadcast_binary_ufunc(op_t op, const lhs_t& lhs, const rhs_t& rhs)
    {
        if constexpr (is_maybe_v<lhs_t>) {
            ...
            using l_lhs_t = get_either_left_t<lhs_t>;
            using l_rhs_t = get_either_right_t<lhs_t>;
            using lhs_ufunc_t = decltype(broadcast_binary_ufunc(op,declval<l_lhs_t>(),rhs));
            using rhs_ufunc_t = decltype(broadcast_binary_ufunc(op,declval<l_rhs_t>(),rhs));
            using result_t = replace_either_t<lhs_t,lhs_ufunc_t,rhs_ufunc_t>;
            if (auto lptr = nmtools::get_if<l_lhs_t>(&lhs)) {
                static_assert( is_pointer_v<decltype(lptr)>
                    , "nmtools internal error" );
                static_assert( is_same_v<remove_cvref_t<decltype(*lptr)>,l_lhs_t>
                    , "nmtools internal error" );
                ...
                static_assert( is_pointer_v<decltype(rptr)>
                    , "nmtools internal error" );
                static_assert( is_same_v<remove_cvref_t<decltype(*rptr)>,l_rhs_t>
                    , "nmtools internal error" );
            ...
            using r_lhs_t = get_either_left_t<rhs_t>;
            using r_rhs_t = get_either_right_t<rhs_t>;
            using lhs_ufunc_t = decltype(broadcast_binary_ufunc(op,lhs,declval<r_lhs_t>()));
            using rhs_ufunc_t = decltype(broadcast_binary_ufunc(op,lhs,declval<r_rhs_t>()));
            using result_t = replace_either_t<rhs_t,lhs_ufunc_t,rhs_ufunc_t>;
            if (auto lptr = nmtools::get_if<r_lhs_t>(&rhs)) {
                static_assert( is_pointer_v<decltype(lptr)>
                    , "nmtools internal error" );
                static_assert( is_same_v<remove_cvref_t<decltype(*lptr)>,r_lhs_t>
                    , "nmtools internal error" );
                ...
                static_assert( is_pointer_v<decltype(rptr)>
                    , "nmtools internal error" );
                static_assert( is_same_v<remove_cvref_t<decltype(*rptr)>,r_rhs_t>
                    , "nmtools internal error" );
     ...
     * @return constexpr auto 
     */
    template <typename op_t, typename array_t, typename axis_t, typename dtype_t=none_t, typename initial_t=none_t, typename keepdims_t=false_type>
    constexpr auto reduce(op_t op, const array_t& array, const axis_t& axis, dtype_t dtype=dtype_t{}, initial_t initial=initial_t{}, keepdims_t keepdims=keepdims_t{})
    {
        ...
        if constexpr (is_maybe_v<array_t>) {
            ...
            using result_type = decltype(reduce(op,declval<array_type>(),axis,dtype,initial,keepdims));
            ...
            using left_t  = get_either_left_t<array_t>;
    ...
    constexpr auto accumulate(op_t op, const array_t& array, axis_t axis, dtype_t dtype=dtype_t{})
    {
        if constexpr (is_maybe_v<array_t>) {
            using array_type  = get_maybe_type_t<array_t>;
            using result_type = decltype(accumulate(op,declval<array_type>(),axis,dtype));
            static_assert( !is_maybe_v<result_type> );
            using return_type = nmtools_maybe<result_type>;
            if (static_cast<bool>(array)) {
    ...
    {
        if constexpr (is_maybe_v<lhs_t>) {
            using lhs_type = get_maybe_type_t<lhs_t>;
            using result_type = decltype(outer(op,declval<lhs_type>(),rhs,dtype));
            using return_type = conditional_t<
                is_maybe_v<result_type>
                , result_type
            ...
            using rhs_type = get_maybe_type_t<rhs_t>;
            using result_type = decltype(outer(op,lhs,declval<rhs_type>(),dtype));
            static_assert( !is_maybe_v<result_type> );
            using return_type = nmtools_maybe<result_type>;

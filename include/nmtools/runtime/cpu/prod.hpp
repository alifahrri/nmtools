#ifndef NMTOOLS_RUNTIME_CPU_PROD_HPP
#define NMTOOLS_RUNTIME_CPU_PROD_HPP
...
{
    class cpu_prod : public base_functor
    {
    public:
    ...
    private:
        ...
        bool  keepdims_;
    public:
        cpu_prod(DType dtype, bool keepdims);
...
#ifdef NMTOOLS_RUNTIME_CPU_PROD_IMPLEMENTATION

#include "nmtools/context/default.hpp"
#include "nmtools/kernel/prod.hpp"
...
namespace nmtools::runtime
{
    cpu_prod::cpu_prod(DType dtype, bool keepdims)
        : dtype_(dtype)
        , keepdims_(keepdims)
    {
        ...
        template_for<N>([&](auto i){
            ...
            if (dtype == this->dtype_) {
                ...
                if (keepdims) {
                    auto keepdims = True;
                    auto view     = unwrap(view::prod(inp.view(dtype),axis,None,initial,keepdims));
    ...
    auto cpu_prod::operator()(const nmtools_list<ndarray>& operands) -> ndarray
    {
        nmtools_panic( operands.size() == 1
            , "invalid number of operand for prod, expect 1"
        );
            ...
            if (dtype == this->dtype_) {
                template_for<M>([&](auto j){
                    ...
                    if ((inp.dim() == nm_size_t{decltype(input_dims)::value}) && (keepdims == this->keepdims_)) {
                        ...
                        auto view      = view::prod(inp.view(dtype,inp_shape),axis,None,initial,keepdims);
                        ...
                        kernel::prod(ctx,k_output,inp.reshape(k_inp_shape).view(dtype,k_inp_shape),tile_shape,-1_ct);
    ...
    auto cpu_prod::graph() const noexcept -> Graph
    {
        return graph_;
    }

    auto cpu_prod::hash() const noexcept -> nm_size_t
    {
        return hash_;
    }
}
#endif // NMTOOLS_RUNTIME_CPU_PROD_IMPLEMENTATION

#endif // NMTOOLS_RUNTIME_CPU_PROD_HPP

#ifndef NMTOOLS_RUNTIME_CPU_ADD_HPP
#define NMTOOLS_RUNTIME_CPU_ADD_HPP
...
{
    class cpu_add : public base_functor
    {
    public:
    ...
    private:
        ...
        Graph graph_;
    public:
        cpu_add(DType lhs_dtype, DType rhs_dtype);
...
#ifdef NMTOOLS_RUNTIME_CPU_ADD_IMPLEMENTATION
...
namespace nmtools::runtime
{
    cpu_add::cpu_add(DType lhs_dtype, DType rhs_dtype)
        : lhs_dtype(lhs_dtype)
        , rhs_dtype(rhs_dtype)
    {
        ...
        template_for<N>([&](auto i){
            ...
            if ((lhs_rtype == this->lhs_dtype) && (rhs_rtype == this->rhs_dtype)) {
                ...
                // runtime::add for real operands (shape/address are not hashed)
                auto lhs = ndarray(IndexType{1,1},this->lhs_dtype);
                auto rhs = ndarray(IndexType{1,1},this->rhs_dtype);
                auto v = unwrap(view::add(lhs.view(lhs_dtype),rhs.view(rhs_dtype)));
    ...
    auto cpu_add::operator()(const nmtools_list<ndarray>& operands) -> ndarray
    {
        nmtools_panic( operands.size() == 2
            , "invalid number of operand for add, expect 2"
        );
        ...
        template_for<N>([&](auto i){
            ...
            if ((lhs_dtype == this->lhs_dtype)
                ...
                kernel::add(ctx,output,lhs.view(lhs_dtype),rhs.view(rhs_dtype),tile_shape,padding);
                array = result;
            }
        });
    ...
    auto cpu_add::graph() const noexcept -> Graph
    {
        return graph_;
    }

    auto cpu_add::hash() const noexcept -> nm_size_t
    {
        return hash_;
    }
}
#endif // NMTOOLS_RUNTIME_CPU_ADD_IMPLEMENTATION

#endif // NMTOOLS_RUNTIME_CPU_ADD_HPP
#ifndef NMTOOLS_RUNTIME_CPU_DIVIDE_HPP
#define NMTOOLS_RUNTIME_CPU_DIVIDE_HPP
...
{
    class cpu_divide : public base_functor
    {
    public:
    ...
    private:
        ...
        Graph graph_;
    public:
        cpu_divide(DType lhs_dtype, DType rhs_dtype);
...
#ifdef NMTOOLS_RUNTIME_CPU_DIVIDE_IMPLEMENTATION

#include "nmtools/context/default.hpp"
#include "nmtools/kernel/divide.hpp"
...
namespace nmtools::runtime
{
    cpu_divide::cpu_divide(DType lhs_dtype, DType rhs_dtype)
        : lhs_dtype(lhs_dtype)
        , rhs_dtype(rhs_dtype)
    {
        ...
        template_for<N>([&](auto i){
            ...
            if ((lhs_rtype == this->lhs_dtype) && (rhs_rtype == this->rhs_dtype)) {
                ...
                // runtime::divide for real operands (shape/address are not hashed)
                auto lhs = ndarray(IndexType{1,1},this->lhs_dtype);
                auto rhs = ndarray(IndexType{1,1},this->rhs_dtype);
                auto v = unwrap(view::divide(lhs.view(lhs_dtype),rhs.view(rhs_dtype)));
    ...
    auto cpu_divide::operator()(const nmtools_list<ndarray>& operands) -> ndarray
    {
        nmtools_panic( operands.size() == 2
            , "invalid number of operand for divide, expect 2"
        );
        ...
        template_for<N>([&](auto i){
            ...
            if ((lhs_dtype == this->lhs_dtype)
                ...
                kernel::divide(ctx,output,lhs.view(lhs_dtype),rhs.view(rhs_dtype),tile_shape,padding);
                array = result;
            }
        });
    ...
    auto cpu_divide::graph() const noexcept -> Graph
    {
        return graph_;
    }

    auto cpu_divide::hash() const noexcept -> nm_size_t
    {
        return hash_;
    }
}
#endif // NMTOOLS_RUNTIME_CPU_DIVIDE_IMPLEMENTATION

#endif // NMTOOLS_RUNTIME_CPU_DIVIDE_HPP

#ifndef NMTOOLS_RUNTIME_CPU_SUBTRACT_HPP
#define NMTOOLS_RUNTIME_CPU_SUBTRACT_HPP
...
{
    class cpu_subtract : public base_functor
    {
    public:
    ...
    private:
        ...
        Graph graph_;
    public:
        cpu_subtract(DType lhs_dtype, DType rhs_dtype);
...
#ifdef NMTOOLS_RUNTIME_CPU_SUBTRACT_IMPLEMENTATION

#include "nmtools/context/default.hpp"
#include "nmtools/kernel/subtract.hpp"
...
namespace nmtools::runtime
{
    cpu_subtract::cpu_subtract(DType lhs_dtype, DType rhs_dtype)
        : lhs_dtype(lhs_dtype)
        , rhs_dtype(rhs_dtype)
    {
        ...
        template_for<N>([&](auto i){
            ...
            if ((lhs_rtype == this->lhs_dtype) && (rhs_rtype == this->rhs_dtype)) {
                ...
                // runtime::subtract for real operands (shape/address are not hashed)
                auto lhs = ndarray(IndexType{1,1},this->lhs_dtype);
                auto rhs = ndarray(IndexType{1,1},this->rhs_dtype);
                auto v = unwrap(view::subtract(lhs.view(lhs_dtype),rhs.view(rhs_dtype)));
    ...
    auto cpu_subtract::operator()(const nmtools_list<ndarray>& operands) -> ndarray
    {
        nmtools_panic( operands.size() == 2
            , "invalid number of operand for subtract, expect 2"
        );
        ...
        template_for<N>([&](auto i){
            ...
            if ((lhs_dtype == this->lhs_dtype)
                ...
                kernel::subtract(ctx,output,lhs.view(lhs_dtype),rhs.view(rhs_dtype),tile_shape,padding);
                array = result;
            }
        });
    ...
    auto cpu_subtract::graph() const noexcept -> Graph
    {
        return graph_;
    }

    auto cpu_subtract::hash() const noexcept -> nm_size_t
    {
        return hash_;
    }
}
#endif // NMTOOLS_RUNTIME_CPU_SUBTRACT_IMPLEMENTATION

#endif // NMTOOLS_RUNTIME_CPU_SUBTRACT_HPP

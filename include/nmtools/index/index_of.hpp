...
namespace nmtools::meta
{
    ...
    template <typename list_t, typename value_t, typename return_multi_t>
    struct resolve_optype<
        void, index::index_of_t, list_t, value_t, return_multi_t
    > {
        static constexpr auto vtype = [](){
            if constexpr (!(is_index_array_v<list_t> || has_value_type_v<list_t>)
                || !(is_index_v<value_t> || is_index_array_v<value_t>)
                || !is_constant_index_v<return_multi_t>
            ) {
                using type = error::INDEX_OF_UNSUPPORTED<list_t,value_t>;
                return as_value_v<type>;
            } else if constexpr (is_constant_index_array_v<list_t>
                && (is_constant_index_array_v<value_t> || is_constant_index_v<value_t>)
            ) {
                constexpr auto list     = to_value_v<list_t>;
                constexpr auto value    = to_value_v<value_t>;
                constexpr auto multi    = return_multi_t{};
                constexpr auto m_result = index::index_of(list,value,multi);
                ...
                // TODO: if value is index array then multi must be false
                [[maybe_unused]]
                ...
                && is_true_type_v<return_multi_t>
            ) {
                constexpr auto B_DIM = max_len_v<list_t>;

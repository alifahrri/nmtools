...
namespace nmtools::index
{
    ...
    constexpr auto remove_dims(const shape_t& shape, const axis_t& m_axis, [[maybe_unused]] keepdims_t keepdims)
    {
        ...
        using return_t = resolve_optype_t<remove_dims_t,shape_t,axis_t,keepdims_t>;
        ...
        auto in_axis = [&]([[maybe_unused]] auto i){
            if constexpr (is_index_v<axis_t>) {
                ...
                auto f_predicate = [i](auto axis){
                    using common_t = promote_index_t<decltype(axis),decltype(i)>;
                    return (common_t)i==(common_t)axis;
                };
        ...
        // , may be using get_index_type_t
        // use the same type as axis_t for loop index
        [[maybe_unused]]
        constexpr auto idx_vtype = [](){
            if constexpr (is_constant_index_array_v<axis_t>) {
                // shortcut for now, just use int
                return as_value_v<int>;
            } else if constexpr (is_index_array_v<axis_t>) {
                using type = get_element_type_t<axis_t>;
                return as_value_v<type>;
            } else if constexpr (is_integer_v<axis_t>) {
                return as_value_v<axis_t>;
            } else {
                return as_value_v<int>;
            }
        }();
        ...
        if constexpr (!is_constant_index_array_v<return_t> && !is_none_v<return_t>) {
            using idx_t = type_t<decltype(idx_vtype)>;
            idx_t idx = 0;

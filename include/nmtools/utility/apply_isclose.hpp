...
namespace nmtools::utils
{
    ...
    constexpr inline auto apply_isclose(const left_t& left, const right_t& right) -> bool
    {
        if constexpr (meta::is_maybe_v<left_t> && meta::is_maybe_v<right_t>) {
            ...
            if (same_null == true) {
                equal = apply_isclose(*left,*right);
            }
            return equal;
            ...
            if (static_cast<bool>(left)) {
                return apply_isclose(*left,right);
            } else {
                return false;
            }
            ...
            return utils::isclose(left,right);
            ...
            for (size_t i=0; (i<len(left)) && (i<len(right)); i++) {
                equal = equal && apply_isclose(at(left,i),at(right,i));
            }
            return equal;
...
#endif // NMTOOLS_UTILS_APPLY_ISCLOSE_HPP
...
namespace nmtools::curvefit
{
    namespace helper
    {
         ...
         */
        template <typename IndexContainer, typename ValueContainer>
        constexpr auto tridiag(const IndexContainer &x, const ValueContainer &y)
        {
            static_assert(
                traits::all_iterable_v<IndexContainer,ValueContainer> && 
                traits::all_indexable_v<IndexContainer,ValueContainer>,
            ...
            >;
            mpl::copy_std_container_t<IndexContainer, value_type> e{}, f{}, g{}, r{};
            auto n = std::size(x)-1;
            if constexpr (traits::is_resizeable<IndexContainer>::value) {
                e.resize(n);
                f.resize(n);
                g.resize(n);
                r.resize(n);
    ...
    template <typename IndexContainer, typename ValueContainer, typename EContainer, typename FContainer, 
        typename GContainer, typename RContainer, typename DContainer, typename Scalar>
    constexpr auto cubic_spline(const IndexContainer &x, const ValueContainer &y, const EContainer &e, const FContainer &f, 
        const GContainer &g, const RContainer &r, const DContainer &d2x, const Scalar &s)
    {
        static_assert(
            traits::all_iterable_v<IndexContainer,ValueContainer,EContainer,GContainer,RContainer,DContainer> &&
            traits::all_indexable_v<IndexContainer,ValueContainer,EContainer,GContainer,RContainer,DContainer>,
     ...
     * @tparam IndexContainer 
     * @tparam ValueContainer 
     * @tparam OutIdxContainer 
     * @param x index/horizontal data points
     * @param y value data points
     * @param t output index point
     * @return constexpr auto results interpolated value for each point in t
     * @reference Sec. Spline Interpolation, Ch. Interpolation
     *      @book{Chapra2010,
                author = {Chapra, Steven C and Canale, Raymond P},
                title = {{Numerical Methods for Engineers Sixth Edition Chapra Canale}},
            } 
     */
    template <typename IndexContainer, typename ValueContainer, typename OutIdxContainer>
    constexpr auto cubic_spline(const IndexContainer &x, const ValueContainer &y, const OutIdxContainer &t)
    {
        assert(std::size(x)==std::size(y));
        using value_type = std::common_type_t<
            typename IndexContainer::value_type,
            typename ValueContainer::value_type,
            typename OutIdxContainer::value_type
        >;
        using result_t = mpl::copy_std_container_t<ValueContainer, value_type>;
        result_t results{};
        if constexpr (traits::is_resizeable<ValueContainer>::value) 

#ifndef NMTOOLS_ARRAY_EVAL_OPENCL_KERNELS_FLATTEN_HPP
#define NMTOOLS_ARRAY_EVAL_OPENCL_KERNELS_FLATTEN_HPP
...
{ \
    ...
    auto result = view::flatten(input); \
...
extern unsigned char nm_cl_flatten_spv[];
extern unsigned int nm_cl_flatten_spv_len;

namespace nmtools::array::opencl
{
    template <typename...args_t>
    struct kernel_t<
        view::decorator_t<view::flatten_t,args_t...>
    > {
        using view_t = view::decorator_t<view::flatten_t,args_t...>;
        ...
        static auto get_spirv()
        {
            ...
            auto spirv = vector();
            spirv.resize(nm_cl_flatten_spv_len);
            memcpy(spirv.data(),nm_cl_flatten_spv,sizeof(unsigned char)*nm_cl_flatten_spv_len);
            return spirv;
        }
...
#endif // NMTOOLS_ARRAY_EVAL_OPENCL_KERNELS_FLATTEN_HPP
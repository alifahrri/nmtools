...
namespace nmtools::view
{
    ...
    constexpr auto subtract(const left_t& a, const right_t& b, casting_t=casting_t{})
    {
        ...
        if constexpr (cast_kind == Casting::AUTO) {
            if constexpr (broadcast_enable) {
                ...
                return binary_ufunc(subtract_t<>{},a,b);
            }
            ...
            if constexpr (broadcast_enable) {
                ...
                return binary_ufunc(subtract_t<lhs_t,rhs_t,rhs_t>{},a,b);
            }
        }
    ...
    constexpr auto reduce_subtract(const left_t& a, const axis_t& axis, dtype_t dtype=dtype_t{}, initial_t initial=initial_t{}, keepdims_t keepdims=keepdims_t{})
    {
    ...
    constexpr auto accumulate_subtract(const left_t& a, axis_t axis, dtype_t dtype=dtype_t{})
    {
        using res_t = get_dtype_t<dtype_t>;
        using op_t  = subtract_t<none_t,none_t,res_t>;
...
namespace nmtools
{
    ...
    constexpr inline auto subtract(const left_t& a, const right_t& b,
    ...
    template <typename output_t=none_t, typename context_t=default_context_t<>,
        typename left_t, typename right_t>
    constexpr inline auto subtract(const left_t& a, const right_t& b, casting::same_kind_t,
        context_t&& context=context_t{}, output_t&& output=output_t{})
    {
    ...
    constexpr auto reduce_subtract(const left_t& a, const axis_t& axis, dtype_t dtype, initial_t initial
        , context_t&& context)
    {
        auto subtract = view::reduce_subtract(a,axis,dtype,initial);
        return eval(subtract
            , nmtools::forward<context_t>(context)
        );
    } // reduce_subtract
    ...
    constexpr auto reduce_subtract(const left_t& a, const axis_t& axis, dtype_t dtype
        , context_t&& context)
    {
        auto subtract = view::reduce_subtract(a,axis,dtype);
        return eval(subtract
            , nmtools::forward<context_t>(context)
        );
    } // reduce_subtract
    ...
    constexpr auto reduce_subtract(const left_t& a, const axis_t& axis
        , context_t&& context)
    {
        auto subtract = view::reduce_subtract(a,axis);
        return eval(subtract
            , nmtools::forward<context_t>(context)
        );
    } // reduce_subtract
    ...
    template <typename context_t
        ...
        , typename axis_t
        , enable_if_t<is_context_v<context_t>,int> = 0>
    constexpr auto accumulate_subtract(const left_t& a, const axis_t& axis
        , context_t&& context)
    {
        auto subtract = view::accumulate_subtract(a,axis);
        return eval(subtract
            , nmtools::forward<context_t>(context)
        );
    } // accumulate_subtract
    ...
    template <typename context_t
        ...
        , typename right_t
        , enable_if_t<is_context_v<context_t>,int> = 0>
    constexpr auto outer_subtract(const left_t& a, const right_t& b
        , context_t&& context)
    {
        auto subtract = view::outer_subtract(a,b);
        return eval(subtract
            , nmtools::forward<context_t>(context)
        );
    } // outer_subtract

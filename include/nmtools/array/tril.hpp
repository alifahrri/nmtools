#ifndef NMTOOLS_ARRAY_VIEW_TRIL_HPP
...
namespace nmtools::index
{
    ...
    constexpr auto shape_tril([[maybe_unused]] const src_shape_t& src_shape)
    {
        using result_t = meta::resolve_optype_t<shape_tril_t,src_shape_t>;
    ...
    constexpr auto tril(const src_shape_t& src_shape, const indices_t& indices, k_t k)
    {
        using result_t = meta::resolve_optype_t<tril_t,src_shape_t,indices_t,k_t>;
...
namespace nmtools::meta
{
    ...
    {
        template <typename...>
        struct SHAPE_TRIL_UNSUPPORTED : detail::fail_t {};

        template <typename...>
        struct TRIL_UNSUPPORTED : detail::fail_t {};
    }

    template <typename src_shape_t>
    struct resolve_optype<
        void, index::shape_tril_t, src_shape_t
    > {
        static constexpr auto vtype = [](){
            if constexpr (!is_index_array_v<src_shape_t>) {
                using type = error::SHAPE_TRIL_UNSUPPORTED<src_shape_t>;
                return as_value_v<type>;
            } else if constexpr (is_constant_index_array_v<src_shape_t>) {
                constexpr auto src_shape = to_value_v<src_shape_t>;
                constexpr auto result = index::shape_tril(src_shape);
    ...
    template <typename src_shape_t, typename indices_t, typename k_t>
    struct resolve_optype<
        void, index::tril_t, src_shape_t, indices_t, k_t
    > {
        static constexpr auto vtype = [](){
            if constexpr (
                ...
                using type = error::TRIL_UNSUPPORTED<src_shape_t,indices_t,k_t>;
                return as_value_v<type>;
            } else {
                constexpr auto DIM = len_v<src_shape_t>;
                [[maybe_unused]]
...
namespace nmtools::view
{
    ...
    struct tril_t
        : base_indexer_t<tril_t<src_shape_t,k_t,src_size_t>>
    {
        ...
        using dst_shape_type = meta::resolve_optype_t<index::shape_tril_t,src_shape_type>;
        ...
        constexpr tril_t(const src_shape_t& src_shape
            , k_t k
            , src_size_t src_size
        )
            : src_shape(fwd_attribute(src_shape))
            , k(fwd_attribute(k))
            , dst_shape(index::shape_tril(src_shape))
            , src_size(src_size)
        ...
        {
            auto src_indices = index::tril(src_shape,indices,k);
            using src_indices_t = meta::get_maybe_type_t<decltype(src_indices)>;
            // TODO: chane index::tril to return either<indices,None>
        ...
        constexpr auto operator==(tril_t<args_t...> other) const
        {
    ...
    template <typename src_shape_t, typename k_t, typename src_size_t>
    constexpr auto tril_indexer(const src_shape_t& src_shape, k_t k, src_size_t src_size)
    {
        if constexpr (meta::is_maybe_v<src_shape_t>) {
            using result_t = decltype(tril_indexer(unwrap(src_shape),k,src_size));
            using return_t = meta::conditional_t<meta::is_maybe_v<result_t>,result_t,nmtools_maybe<result_t>>;
            return (src_shape
                ? return_t{tril_indexer(unwrap(src_shape),k,src_size)}
                : return_t{meta::Nothing}
            );
        } else {
            return tril_t{src_shape,k,unwrap(src_size)};
        }
    } // tril_indexer
    ...
    constexpr auto tril(const array_t& array, k_t k=k_t{0})
    {
        auto f = [](const auto& array, auto k){
            ...
            auto indexer    = tril_indexer(src_shape,k,src_size);

#ifndef NMTOOLS_ARRAY_EVAL_OPENCL_KERNELS_FLIP_HPP
#define NMTOOLS_ARRAY_EVAL_OPENCL_KERNELS_FLIP_HPP
...
#define nmtools_cl_kernel_name(out_type,inp_type) flip_##out_type##_##inp_type
#define nmtools_cl_kernel_name_str(out_type,inp_type) nm_stringify(flip_##out_type##_##inp_type)
...
kernel void nmtools_cl_kernel_name(out_type,inp_type) \
    ...
    , global const nm_cl_index_t* axis_ptr \
    , const nm_cl_size_t out_dim \
    , const nm_cl_size_t inp_dim \
    , const nm_cl_size_t axis_len \
    ) \
{ \
    auto axis  = na::create_vector(axis_ptr,axis_len); \
    ...
    auto result = view::flip(input,axis); \
...
extern unsigned char nm_cl_flip_spv[];
extern unsigned int nm_cl_flip_spv_len;

namespace nmtools::array::opencl
{
    template <typename...args_t>
    struct kernel_t<
        view::decorator_t<view::flip_t,args_t...>
    > {
        using view_t = view::decorator_t<view::flip_t,args_t...>;
        ...
        static auto get_spirv()
        {
            ...
            auto spirv = vector();
            spirv.resize(nm_cl_flip_spv_len);
            memcpy(spirv.data(),nm_cl_flip_spv,sizeof(unsigned char)*nm_cl_flip_spv_len);
            return spirv;
        }
        ...
        auto eval(output_t& output)
        {
            ...
            // assume index array
            auto axis = index::cast<nm_cl_index_t>(view.axes);
            auto axis_len = nmtools::len(axis);

            auto axis_buffer = context->create_buffer(axis);
...
#endif // NMTOOLS_ARRAY_EVAL_OPENCL_KERNELS_FLIP_HPP